...
Ou com o script auxiliar:

```powershell
.\tools\rtp.ps1 test_predict
```

Os testes usam um mock do modelo (sem carregar o `.pkl`), cobrindo: shape da resposta, campos obrigatórios, lógica do threshold, passthrough do `student_id` e rejeição de payloads inválidos (HTTP 422).

---

## Script `rtp.ps1`

Utilitário de desenvolvimento para localizar e executar qualquer script `.py` do projeto sem precisar saber o caminho completo.

```powershell
.\tools\rtp.ps1 <nome_parcial>

# Exemplos:
.\tools\rtp.ps1 generate      # executa scripts/generate_data.py
.\tools\rtp.ps1 train         # executa scripts/train_model.py
.\tools\rtp.ps1 test          # exibe menu se houver múltiplos arquivos com "test"
```

Se houver mais de um arquivo correspondendo ao padrão, um menu numerado é exibido para desambiguação.
